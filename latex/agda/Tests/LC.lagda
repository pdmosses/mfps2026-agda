...
%
A reviewer pointed out that the only interpretation of \AgdaRef{⟪\ D∞\ ⟫}
in Agda could be a singleton type, so that one should expect many
equalities to hold. However, when the proof of an equality is simply by
\AgdaRef{refl}, Agda's type-checker does not automatically use such
reasoning.
