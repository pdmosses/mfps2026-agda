...
[Denotational Semantics] recalls the main concepts and notation of Scott–Strachey denotational semantics.
[Agda] highlights features of the Agda language whose use or meaning might not be immediately obvious.

[Denotational Semantics]: Background.md#denotational-semantics
[Agda]: Background.md#agda
...
The arguments of semantic functions are enclosed by $\llbracket\dots\rrbracket$,
to avoid potential confusion between abstract syntax terms and semantic notation.

Domains were originally continuous lattices [(Scott1970OMT)]\ [(Scott1972CL)]\ [(Stoy1977DSS)]\ [(Tennent1976DSP)].
...
Let $D, E$ be domains, $\delta : D$, and $\epsilon : E$.
...
- A function $\phi : D \to E$ is (Scott-) continuous if it is monotone and
  ...
  A function domain $F = D \to E$ consists of continuous (total) functions from $D$ to $E$.
  The set of all functions from a set $A$ to a domain also forms a domain, ordered point-wise.
  Functions between domains are defined using abstraction $\lambda \delta.\epsilon$, application $\phi\,\delta$,
  the operation $\texttt{fix}$ that maps each $\phi : D \to D$ to its least fixed point,
  and the operations associated with other domain constructors.

- For any set $A$ the flat domain $A_\bot$ is formed by adding $\bot$ to $A$.
...
- A separated sum domain $X = \ldots + Y + \ldots$ consists of injected elements
...
Bob Tennent's highly accessible tutorial on denotational semantics [(Tennent1976DSP)]
includes illustrative examples (partly in continuation-passing style) and
further details of the conventional Scott–Strachey style.

## Agda

Agda [(Agda-Language)]\ [(Agda-Wikipedia)] is both a strongly typed
functional programming language with support for first-class *dependent
types* and a *proof assistant* based on the Curry-Howard correspondence
between propositions and types. A number of features set Agda apart from
...
Anonymous functions.
:   Lambda expressions use the syntax `λ x → u` instead of `λ x. u`.
...
Rewrite rules.
:   Agda has support for *rewrite rules* [(cockx:LIPIcs.TYPES.2019.2)]\ [(Cockx2021TRT)]
    that are applied automatically during type checking.
    Rewrite rules are declared by marking proofs of the equality type `a ≡ b`
    with a `REWRITE` pragma.
